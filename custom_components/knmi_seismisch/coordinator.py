...
class KNMISeismischCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        if self._is_first_run and self.last_data:
            ...
            _LOGGER.debug(
                "Eerste run na opstarten: KNMI Download overgeslagen, cache gebruikt."
            )
        ...
        try:
            async with session.get(URL_KNMI) as response:
                ...
                for event in root.iter("event"):
                    ...
                    display_region = (
                        region
                        if region != display_location
                        else "Regio niet gespecificeerd"
                    )
                    ...
                    if self.search_terms and not any(
                        term in display_location.lower() or term in region.lower()
            ...
            debug_path = os.path.join(
                current_dir, f"knmi_debug_{self.instance_name}.txt"
            )
